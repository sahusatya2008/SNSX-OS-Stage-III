...
- SNSX Browser app for bundled pages, bookmarks, network-status-aware browsing, and plain HTTP page fetches
...
2. Start the created VM in VirtualBox
3. Let the diagnostics screen auto-continue or press `Enter`
4. At `SNSX First-Run Setup`, choose `INSTALL` and then `SAVE`, or just wait for auto-desktop
5. Choose `DESKTOP` if it has not already auto-opened
6. In a second host terminal, run `make https-bridge` if you want `https://` browsing in VirtualBox
   If it says the bridge is already running, that is fine. Use `make https-bridge-status` to inspect it or `make https-bridge-stop` to restart it.
7. Open `Terminal` inside SNSX and run `netconnect`, `ip`, `dns example.com`, `http http://example.com/`, and `http https://example.com/`
8. Power off the VM
