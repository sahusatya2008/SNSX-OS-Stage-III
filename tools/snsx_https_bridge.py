        ...
        try:
            with urllib.request.urlopen(request, timeout=20, context=context) as response:
                ...
                if not (content_type.startswith("text/") or content_type in ("application/json", "application/xml")):
                    self._write_text(
                        415,
                        status_line + f"Unsupported content type for SNSX browser bridge: {content_type}\n",
                    )
                    return

                raw = response.read(MAX_BYTES)
                text = raw.decode(charset, errors="replace")
                body = status_line + f"Source: {target}\nContent-Type: {content_type}\n\n" + text
                self._write_text(200, body, content_type="text/plain; charset=utf-8")
